...
public:
    ...
    , rho_(N_tot)
    {
        ...
        cudaMalloc(&d_rho_, N_tot*sizeof(float));
        cudaMemset (&d_rho_, 0,N_tot*sizeof(float));
        cudaMalloc(&d_rho_tmp_, N_tot*sizeof(float));
    ...
    float GetMoment() {
        ...
        cudaMemcpy(&rho_[0],d_rho_,rho_.size() * sizeof(float), cudaMemcpyDeviceToHost);
...
void Diffusion2D::PropagateDensity(int steps)
{
    ...
    /// in time
    int grid_size = std::floor(log2(N_)/4);
    grid_size = pow(2,grid_size+1);
    int block_size = std::floor(log2(N_)*3/4);//num of threads
    block_size = pow(2, block_size);
    //std::cout << "Number of Blocks : " << grid_size <<'\n' << "Threads per Block : " << block_size << "\n" << "Number of Threads : " << grid_size*block_size <<"\n";
